# Check if the `marg_liks` are of the same type as returned by [est_marg_liks](https://docs.ropensci.org/mcbette/reference/est_marg_liks.md).
...
- marg_liks:
  ...
  to determine if a `marg_liks` is valid. Use check_marg_liks to check
  that a `marg_liks` is valid.
